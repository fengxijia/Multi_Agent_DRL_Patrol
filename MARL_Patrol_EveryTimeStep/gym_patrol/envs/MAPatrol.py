...
import imageio
# import model_patrol as model_patrol
# from model_patrol import Net
import torch

# import sys
# sys.path.append('E:/EE5003/MARL_Patrol-dev-jiawei/')# algorithms/ppo.py

# from algorithms.ppo import *
from collections import deque
# from buffers.rollout_buffer import *
import os
import shutil
import argparse
import configparser
...
class MAPatrolEnv(gym.Env):
    ...
    def __init__(self, numAgents = 4, numTargets_range=(10, 10), env_size=(1.0, 1.0), min_dist=None, nn_range=(3, 5), comms_radius=None):
        """
    ...
    def step(self, ID, next_target_ind, relative=True, localReward=True): 
        ...
        tmpRwrd = self.agents[ID-1].move() # 更新agent此时位置的reward指标即idleness（不是reward）,若在点上返回idleness,在边上返回-1
        ...
        if localReward: # 环境如何给reward
            ...
            return agentStates, targetStates, connectivity_feature, localMask, self.agents[ID-1].localTargetCoords, self.agents[ID-1].finished, reward
        else: ###这个是啥意思，一直没用到？
            return agentStates, targetStates, connectivity_feature, localMask, self.agents[ID-1].localTargetCoords, self.agents[ID-1].finished # localMask??
    ...
    def render(self, save=False, show_comms=False):
        ...
        self.targets_sc.set_edgecolor([0., 0., 0., 1.])
        # display
        self.ax.set_title("[step {}|{}] global average idleness: {}, global worst idleness: {}".format(self.elapsed_steps, self.max_episode_steps, round(self.globalIdleness.mean(),2), round(self.globalIdleness.max(),1)))
...
if __name__ == "__main__":
    ...
    numAgents = 4
    numTargets = 10
    env = MAPatrolEnv(numAgents=numAgents, numTargets_range=(numTargets, numTargets))
    ...
    while True:
        ...
        for n in range(1, numAgents+1): 
            a = env.agents[n-1].randomPolicy(False) 
