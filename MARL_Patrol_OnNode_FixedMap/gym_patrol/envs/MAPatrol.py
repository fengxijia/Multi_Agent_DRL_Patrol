...
def generate_points_with_min_distance(n, size, min_dist):
    ...
    x = np.linspace(0., size[1], num_x, dtype=np.float32)
    y = np.linspace(0., size[0], num_y, dtype=np.float32)
    ...
    # left
    left = np.logical_and(coords[:, 0]<max_movement, np.logical_and(coords[:, 1]>=max_movement, coords[:, 1]<=size[0]-max_movement))
    ...
    # top
    top = np.logical_and(np.logical_and(coords[:, 0]>=max_movement, coords[:, 0]<=size[1]-max_movement), coords[:, 1]>size[0]-max_movement)
    low[top], high[top] = -np.pi, 0
    
    # top right
    topright = np.logical_and(coords[:, 0]>size[1]-max_movement, coords[:, 1]>size[0]-max_movement)
    low[topright], high[topright] = -np.pi, -np.pi/2   
    # right
    right = np.logical_and(coords[:, 0]>size[1]-max_movement, np.logical_and(coords[:, 1]>=max_movement, coords[:, 1]<=size[0]-max_movement))
    ...
    # bottom
    bottom = np.logical_and(np.logical_and(coords[:, 0]>=max_movement, coords[:, 0]<=size[1]-max_movement), coords[:, 1]<max_movement)
...
def findEdgeByPos(envMap, pos, next_target_ind, targetCoords):
    ...
    for ind in next_target_nbr_ind:
        ...
        if checkOnInterval(pos, nbr_pos, next_target_pos): # 检查当前坐标，下一target和下一target邻居坐标三点是否共线。其中必有一个是该点所在边的另一个端点
...
class Agent(object):
    ...
    def _initAgent(self):
        self.prev_target_ind = findNodeIndexByPos(self.localMap, self.pos) # 此时的pos是上一时刻的target,取得其index
        nbr_ind = list(self.localMap.neighbors(self.prev_target_ind)) # 邻居的prev_target跟↑一样 findnode
    ...
    def get_target_coords(self, relative=True):
        ...
        # absolute observation
            ...
            target_coords = self.localTargetCoords   # array, size(10, 2) [[x1, y1], [x2, y2], ... , [xn, yn]]
    ...
    def getNbrObs(self, relative=True):
        ...
        '''
        cachedNeighborObs = [(self.pos, self.localTargetCoords[self.next_target_ind])] + \
    ...
    def move(self, next_target_ind):
        
        '''
        Moving to the destination.
        Update self.localIdleness, self.pos
        '''
        self._elapsed_steps += 1
        self.next_target_ind = next_target_ind
        targetPos = self.localTargetCoords[self.next_target_ind] # 目的地坐标
        # targetIdleness = self.localIdleness[self.next_target_ind] # 此刻的idleness暂时变成目标位置的idleness且暂不清零，因为有可能一步到不了
        currPos = self.pos
        ...
        if dis2Target == 0: # 如果我就在目的地上 那我就不动了
            ...
            self.pos = currPos + self.step_length*nvec # 此刻我的位置变成了往前挪一步后的位置，即实现了move
            self.isOnNode = False
        # return targetIdleness
    ...
    def randomPolicy(self, allow_reverse=True):
        ...
        if self.isOnNode:
            ...
            edge = findEdgeByPos(self.localMap, self.pos, self.next_target_ind, self.localTargetCoords)
            if allow_reverse:
    ...
    def __str__(self):
        return "Agent-{}, pos: {}, prev_target_ind: {}, next_target_ind: {}".format(self.ID, self.pos, self.prev_target_ind, self.next_target_ind)
...
class MAPatrolEnv(gym.Env):
    ...
    def __init__(self, numAgents_range = (4, 4), numTargets_range=(10, 10), env_size=(1.0, 1.0), min_dist=None, nn_range=(3, 5), comms_radius=None):
        """
        ...
        """
        super(MAPatrolEnv, self).__init__()
        ...
        self.min_dist = min(self.env_size)/20. if min_dist is None else min_dist
    ...
    ############################
    def params_from_cfg(self, args, cfgs):
        '''
    ...
    def _initEnv(self):
        ...
        self.targetCoords = generate_points_with_min_distance(self.numTargets, self.env_size, self.min_dist)
    ...
    def _buildMap(self, weighted=True):
        ...
        for i in range(self.numTargets):
            ...
            for j in range(1, num_nbr+1):
                G.add_edge(i, neigh_ind[i, j], weight=neigh_dist[i, j]) # add node i, neigh_ind[i,j] and connect these two nodes, add distance as weight
    ...
    def communicate(self, ID):
        ...
        agentPos = np.array([a.pos for a in self.agents]).reshape(-1, 2) # 所有agent的位置包括自己 shape(num_agents,2)
        ownPos = self.agents[ID-1].pos.reshape(1, 2) # 我自己的position, shape(1,2)
        relDiff = ownPos-agentPos # 所有agent和我的坐标差 shape(num_agents,2)
        relDisSqaure = relDiff[:, 0]**2 + relDiff[:, 1]**2 # shape(num_agents,) [0.17,0,0.30,0.40] ID = 2, 所以第1列为0
        nbr_ind = relDisSqaure.argsort() # 如[0.17,0,0.30,0.40] -> [1,0,2,3] 按照距离从小到大排列，不是按照连不连接
        ...
        if self.comms_radius is not None:
            ...
            for nbr in nbr_ind[1:]: # 不包括自己的所有邻居
                if relDisSqaure[nbr] <= self.comms_rad_square: # 如果在通讯范围内
                    self.agents[ID-1].cachedLocalIdleness.append(self.agents[nbr].localIdleness) # 把每个邻居的idlenssmap都append起来
                    nbrObs = (self.agents[nbr].pos,
                              self.agents[ID-1].localTargetCoords[self.agents[nbr].next_target_ind]) # agent state! ([该邻居的坐标], [邻居接下来要去地点的坐标])
                    self.agents[ID-1].cachedNeighborObs.append(nbrObs) # 把每个邻居的Obs也append起来
        ...
        # global communication
            ...
            for nbr in nbr_ind[1:]:
                ...
                nbrObs = (self.agents[nbr].pos, self.agents[ID-1].localTargetCoords[self.agents[nbr].next_target_ind])
                self.agents[ID-1].cachedNeighborObs.append(nbrObs) # 这里更新agent里的self.cachedNeighborObs属性

    def observe(self, ID, relative=True):
        '''
        In the view of agent No. ID, get the target states and agent states. Update GLOBAL idleness when on the node. Return local mask.
        '''
        ...
        target_coords = self.agents[ID-1].get_target_coords(relative) # array, shape(numTargets, 2)
        target_states = np.hstack((target_coords, localIdleness.reshape(-1,1))) # array, shape(numTargets, 3)
        ...
        local_A = nx.adjacency_matrix(self.agents[ID-1].localMap, weight=None).todense() # Adj Max. shape(num_target,num_target)
        if self.agents[ID-1].isOnNode: # 如果这个agent在点上
            target_ind = findNodeIndexByPos(self.agents[ID-1].localMap, self.agents[ID-1].pos) # 找出此刻agent在哪个索引的点上 localMap就是envMap
            # self.globalIdleness[target_ind] = 0 # globalIdleness shape: (num_target,) [0,0,...,0] 把该点的idleness置零
            localMask = np.array((local_A[target_ind] == 1)) # LocalMask记录了哪些点与target_ind这个点连接
        else: # 如果在边上，只能沿着之前的方向继续往前走
            # edge = findEdgeByPos(self.agents[ID-1].localMap, self.agents[ID-1].pos, self.agents[ID-1].next_target_ind, self.agents[ID-1].localTargetCoords) # [ind1, ind2]
            localMask = np.zeros(self.numTargets, dtype=bool)
            localMask[self.agents[ID-1].next_target_ind] = 1
    ...
    def checkAnyOnNode(self):
        return True if np.any([a.isOnNode for a in self.agents]) else False
    ...
    #     targetIdleness = self.agents[ID-1].move() # 更新agent此时位置的reward指标即idleness（不是reward）,若在点上返回idleness,在边上返回-1
    #     agentStates, targetStates, connectivity_feature, localMask = self.observe(ID, relative)
    
    def update_globalIdleness(self, ID):    
        # 更新global idleness
        if self.agents[ID-1].isOnNode: # 如果这个agent在点上
            target_ind = findNodeIndexByPos(self.agents[ID-1].localMap, self.agents[ID-1].pos) # 找出此刻agent在哪个索引的点上 loscalMap就是envMap
            self.globalIdleness[target_ind] = 0 # globalIdleness shape: (num_target,) [0,0,...,0] 把该点的idleness置零
        
    def checkAgentFinished(self, ID):
        # check if agent has finished
        if self.agents[ID-1].elapsed_steps >= self.max_episode_steps:
            self.agents[ID-1].finished = True
    
    def get_agent_reward(self, ID, localReward = True):
        # targetIdleness = self.agents[ID-1]
        targetIdleness = self.agents[ID-1].localIdleness[self.agents[ID-1].next_target_ind] # 此刻的idleness暂时变成目标位置的idleness且暂不清零，因为有可能一步到不了
        if localReward: # 要去的目的地的idleness  
            reward = targetIdleness
        else:
            reward = self.agents[ID-1].localIdleness.sum() / self.numTargets
        return reward / self.max_episode_steps
    ###### replace STEP（）######
    ...
    def reset(self, fixedMap, scattered=False):
        ...
        if not fixedMap:
            ...
            # self.numAgents = self.numAgents_range[0]
            # self.numTargets = self.numTargets_range[0]
            # self.targetCoords = generate_points_with_min_distance(self.numTargets, self.env_size, self.min_dist)
            # self.map = self._buildMap()
            self.globalIdleness = np.zeros(self.numTargets)
            self.connectivity_feature = self.get_connectivity_feature(self.dim_connectivity)
        ...
        if not scattered:
            ...
            self.agents = [Agent(ID, self.targetCoords[depot_ind], self.map, self.targetCoords, np.copy(self.globalIdleness)) for ID in range(1, self.numAgents+1)]
        else:
            pos_ind = np.random.choice(self.numTargets, size=self.numAgents)
            self.agents = [Agent(ID, self.targetCoords[pos_ind[ID-1]], self.map, self.targetCoords, np.copy(self.globalIdleness)) for ID in range(1, self.numAgents+1)]
    ...
    def render(self, save=False, show_comms=False):
        if self.fig is None:
            ...
            # plot targets
            self.targets_sc = self.ax.scatter(self.targetCoords[:, 0], self.targetCoords[:, 1], marker='s', s=10**2)
            self.targets_sc.set_color([1., 0., 0., 0.])
            self.targets_sc.set_edgecolor([0., 0., 0., 1.])
            
            # annotate target indexes
            for i, pos in enumerate(self.targetCoords):
                self.ax.text(pos[0]+self.env_size[1]/100., pos[1]+self.env_size[0]/100., str(i))
            for e in self.map.edges():
                self.ax.plot(*zip(self.targetCoords[e[0]], self.targetCoords[e[1]]), c='k', lw=0.5, ls='-.')
            ...
            if show_comms and self.comms_radius is not None:
                if self.comms_circle is None:
                    ...
                    self.comms_circle = [self.comms_radius*np.cos(np.linspace(0, 2*np.pi, numPlotPoints)), self.comms_radius*np.sin(np.linspace(0, 2*np.pi, numPlotPoints))]
                self.agents_comms_circle_l = []
                for a in self.agents:
                    comms_circle_l, = self.ax.plot(*map(lambda x: sum(x), zip(a.pos, self.comms_circle)), c='g', ls='--', lw=1)
        ...
        if show_comms and self.comms_radius is not None:
            for i, comms_circle_l in enumerate(self.agents_comms_circle_l):
                comms_circle = list(map(lambda x: sum(x), zip(self.agents[i].pos, self.comms_circle)))
                comms_circle_l.set_xdata(comms_circle[0])
                comms_circle_l.set_ydata(comms_circle[1])
        
        # update idleness
        global_idleness_c = [[1., 0., 0., idleness/self.max_episode_steps] for idleness in self.globalIdleness] # different shade of red according to the idleness
        self.targets_sc.set_color(global_idleness_c)
        self.targets_sc.set_edgecolor([0., 0., 0., 1.])
        # display
        self.ax.set_title("[step {}|{}] global average idleness: {}, global worst idleness: {}".format(self.elapsed_steps+1, self.max_episode_steps, round(self.globalIdleness.mean(),2), round(self.globalIdleness.max(),1)))
...
if __name__ == "__main__":
    ...
    numAgents = 4
    numTargets = 10
    env = MAPatrolEnv(numAgents_range = (numAgents, numAgents), numTargets_range=(numTargets, numTargets))
    env._initEnv()
    env.reset(fixedMap = True, scattered=True)
    for agent in env.agents:
        print(agent)
    while True:
        env.render(True, True)
        
        # 再遍历所有agent使得它们都communicate一次
        for n in range(1, numAgents+1): 
            env.communicate(n)
        
        # 遍历所有agent使得所有agent都step一次
        for ID in range(1, numAgents+1): 
            a = env.agents[ID-1].randomPolicy(False) 
            env.agents[ID-1].move(a)
            env.checkAgentFinished()
            # agentStates, targetStates, connectivity_feature, localMask, localTargetCoords, finished, reward = env.step(n, a)
            
        env.updateEnv()
        if env.checkFinished():
            break
        time.sleep(0.1)
    env.save('{}agents-{}targets-{}comms_radius-{}max_steps.gif'.format(env.numAgents, env.numTargets, env.comms_radius if env.comms_radius is not None else 'inf', env.max_episode_steps))
