"""On-node patrolling environment.

Agents make a decision only when they are physically on a target node; while in
transit along an edge they keep moving toward their chosen target. Set
``fixed_map=True`` in :meth:`MAPatrolEnv.reset` to reuse the same graph across
episodes (the former ``OnNode_FixedMap`` variant, now folded in as a flag).
"""
...
class MAPatrolEnv(gym.Env):
    ...
    def update_globalIdleness(self, ID):
        ...
        if self.agents[ID - 1].isOnNode:  # 如果这个agent在点上
            target_ind = findNodeIndexByPos(
                self.agents[ID - 1].localMap, self.agents[ID - 1].pos
            )  # 找出此刻agent在哪个索引的点上 loscalMap就是envMap
    ...
    def get_agent_reward(self, ID, localReward=True):
        ...
        targetIdleness = self.agents[ID - 1].localIdleness[
    ...
    def reset(self, scattered=False, fixed_map=False):
        """Start a new episode.

        Args:
            scattered: if True, agents start on random nodes; otherwise they all
                start from a single random depot node.
            fixed_map: if True, reuse the current graph/targets (only reset the
                idleness); if False, regenerate the whole environment. Reusing
                the map gives reproducible episodes (former ``OnNode_FixedMap``).
        """
        self._elapsed_steps = 0
        self.frames.clear()
        ...
        if not fixed_map:
            ...
            # Keep numAgents / numTargets / targetCoords / map; only refresh the
            # idleness state and recompute the connectivity feature.
            self.globalIdleness = np.zeros(self.numTargets)
            self.connectivity_feature = self.get_connectivity_feature(self.dim_connectivity)
        if not scattered:
    ...
    def render(self, save=False, show_comms=False):
        ...
        # display
        self.ax.set_title(
            f"[step {self.elapsed_steps+1}|{self.max_episode_steps}] global average idleness: {round(self.globalIdleness.mean(),2)}, global worst idleness: {round(self.globalIdleness.max(),1)}"
        )
