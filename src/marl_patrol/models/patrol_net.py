"""Transformer-based actor-critic network ("PatrolNet") for the patrol task.

Part of the code credit to
https://towardsdatascience.com/how-to-code-the-transformer-in-pytorch-24db27c8f9ec

The architecture is unchanged from the original project; only dead imports and a
hardcoded module-level ``device`` were removed (the device is now decided by the
caller and tensors are moved explicitly).
"""
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        concat = (
            output_attention.transpose(1, 2).contiguous().view(bs, -1, self.d_model)
        )  # 4, 1, 16
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, heads, dropout=0.1):  # d_model corresponds to the 'd'
        ...
        self.attn = MultiHeadAttention(
            heads, d_model
        )  # heads, d_model are construction parameters for the MultiHeadAttention layer
    ...
    def forward(
        self, q, k, v
    ):  # self, q, k, v           # x:(10, 10, 16) ae_output: (4, 4, 16) 要normalize y吗，decoderlayer没有normalize
...
class Encoder_TargetAgent(nn.Module):  # combine with the encoder
    ...
    def forward(self, x, ae_output):  # deleted the mask
        for i in range(self.N):
            x = self.layers[i](
                x, ae_output, ae_output
            )  # deleted the mask. x:(10, 10, 16) ae_output: (4, 4, 16)
        return self.norm(x)
...
def index_pooling(ae_output, te_output):
    indexing = ae_output[
        :, :1, :
    ]  # Haven't figured out how to find the current agent embedding from all agent embeddings,
...
class ActorCriticPolicy(nn.Module):
    def __init__(self, cfgs):
        ...
        self.target_encoder = Encoder_Target(
            self.dim_cf, self.dim_tf, self.d_model, self.N, self.heads
        )
...
if __name__ == "__main__":
    # Minimal self-test with a fake config (no external files needed).
    class _Cfg(dict):
        def getint(self, k):
            return int(self[k])

    cfgs = _Cfg(dim_unify=16, repeat_times_inside_net=2, heads=8, dim_af=4, dim_cf=4, dim_tf=3)
    net = ActorCriticPolicy(cfgs)
    num_agents, num_targets, batch_size = 4, 10, 40
    obs = (
        torch.randn(batch_size, num_agents, 4),
        torch.randn(batch_size, num_targets, 3),
        torch.randn(batch_size, num_targets, 4),
        torch.tensor(batch_size * [[[True] * num_targets]]),
    )
